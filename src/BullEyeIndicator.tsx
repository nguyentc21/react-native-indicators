...
import type { BullEyeIndicatorProps, RenderComponentProps } from './types';
...
function BullEyeIndicator(
  ...
  const subWidth = size / 16;
  const subHeight = size / 4;
...
const BullEyeIndicatorMemo = React.memo(
  BullEyeIndicator,
  (p, n) =>
    p.count === n.count &&
    p.index === n.index &&
...
function ActivityIndicator(props: BullEyeIndicatorProps) {
  const {
    color = DEFAULT_COLOR,
    count = DEFAULT_COUNT,
    size = DEFAULT_SIZE,
